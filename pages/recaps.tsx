...
const RecapsPage: NextFunctionComponent<RecapsPageProps> = ({ recaps }) => {
  return (
    <div>
      <nav>
        <Link href="/">
...
function getRecaps(): Promise<RecapsPageProps> {
  return axios
    ...
    .catch((error: AxiosError) => {
      if (error.response) {
        console.log(error.response.status, "failed to retrieve recaps");
      }
      const errorResponse = { recaps: [] };
      return errorResponse;
    });
}

RecapsPage.getInitialProps = async () => {
